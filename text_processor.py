...
class TextProcessor:
    @staticmethod
    def _setup_unclear_logger():
        """Setup logging for unclear classifications"""
        logger = logging.getLogger('unclear_classifications')
        if not logger.handlers:  # Only add handler if it doesn't exist
            logger.setLevel(logging.INFO)
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            handler = logging.FileHandler(f'unclear_classifications_{timestamp}.log', encoding='utf-8')
            formatter = logging.Formatter('%(asctime)s\nOriginal Text: %(original_text)s\nModel Output: %(model_output)s\n\n')
        ...
        try:
            ...
            response = requests.post('http://localhost:11434/api/generate', 
                                   json={
                                       ...
                                       Do not answer with "AI" or "answer" or whatever, output should be only ONE word!
            ...
            if response.status_code == 200:
                    ...
                    # Extract classification after </think> tag if present
                    if '</think>' in result:
                        classification = result.split('</think>')[-1].strip()
                    else:
                        classification = result.strip()
                    
                    if classification in ['fact', 'law', 'compensation']:
                        return classification
                    else:
                        # Log unclear classification
                        unclear_logger = TextProcessor._setup_unclear_logger()
                        unclear_logger.info('', extra={
                            'original_text': chunk,
                            'model_output': result
                        })
                        print(f"Unclear classification result: {classification}, defaulting to 'fact'")
